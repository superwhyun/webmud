...
describe('rollMobLoot', () => {
  it('returns no items when the mob template has no configured loot pool', () => {
    expect(rollMobLoot(TEST_MOB_TEMPLATE_ID)).toEqual([]);
  });

  it('only ever returns item ids that are part of the configured pool', () => {
    db.prepare('INSERT INTO mob_loot_pool (mob_template_id, item_id) VALUES (?, ?)').run(
      TEST_MOB_TEMPLATE_ID,
      TEST_ITEM_ID,
    );

    for (let i = 0; i < 50; i++) {
      const carried = rollMobLoot(TEST_MOB_TEMPLATE_ID);
      expect(carried.length).toBeLessThanOrEqual(2);
  ...
  it('sometimes carries the item and sometimes carries nothing across many rolls', () => {
    ...
    const counts = Array.from({ length: 100 }, () => rollMobLoot(TEST_MOB_TEMPLATE_ID).length);
