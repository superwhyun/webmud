...
interface ZoneBlueprint {
  ...
  roomBase: number;
  /** 이 존 레벨대의 덩굴괴수/불도마뱀/바위골렘/강철전갈/늪지악어 mob_templates id, 이 순서 그대로 전투방 2~6에 배치. */
  mobTemplateIds: [number, number, number, number, number];
...
export const PROGRESSION_MOB_SPAWNS: ProgressionMobSpawn[] = ZONE_BLUEPRINTS.flatMap((zone) =>
  zone.mobTemplateIds.map((mobTemplateId, index) => ({
    // 전투방은 입구(+1) 다음인 +2 ~ +6.
